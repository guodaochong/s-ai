...
\title{\textbf{S-AI}: A Multi-Agent Spatial Intelligence Framework for\\Distributed Hydrological Modeling and Autonomous Flood Assessment\\via LLM-Orchestrated MCP Architecture}
...
\begin{abstract}
Flood disasters constitute one of the most consequential and escalating threats to civil infrastructure and human welfare in the twenty-first century, with global economic losses surpassing \$80 billion per annum and affecting over two billion people between 2000 and 2020. While physics-based hydrodynamic models --- particularly those solving the two-dimensional shallow water equations --- provide the gold standard for inundation prediction, their operational deployment remains encumbered by prohibitive computational overhead, labyrinthine parameter calibration, and a steep learning curve that confines their use to a narrow cadre of domain specialists. This paper introduces \textbf{S-AI} (Spatial-AI), a production-grade, multi-agent spatial intelligence platform that reconceptualizes flood modeling through the lens of Large Language Model (LLM)-orchestrated automation. At its core, S-AI leverages the \emph{Model Context Protocol} (MCP) to federate 42+ specialized geospatial and hydrological tools across eight microservices, enabling natural language intent parsing via GLM-5.1 that autonomously decomposes user queries into executable hydrodynamic workflows.

The platform pioneers a \textbf{distributed hydrological modeling chain} that automates the complete pipeline from rainfall to building-level inundation assessment in under 15 seconds: (i)~global SRTM 90\,m DEM acquisition via Cloud Optimized GeoTIFF windowed reads from AWS CloudFront; (ii)~D8 flow direction and topological-sort flow accumulation for watershed delineation; (iii)~\emph{spatially distributed SCS-CN curve number assignment} driven by OpenStreetMap land use polygons (residential $\to$ CN\,85, commercial $\to$ CN\,92, forest $\to$ CN\,55), achieving CN ranges of 55--98 versus 58--63 from slope-only estimation; (iv)~time-area method flow routing with Manning velocity-based travel time computation; (v)~two-dimensional shallow water inundation via 8-neighbor Manning diffusion; and (vi)~per-building flood impact assessment by sampling peak depth grids against OpenStreetMap building footprints.

Beyond flood simulation, S-AI integrates \textbf{autonomous drone mission planning} that leverages flood simulation outputs to identify risk hotspots, generates optimal inspection waypoints via nearest-neighbor TSP initialization with 2-opt local search, and exports KML files directly executable on DJI flight platforms. The system further incorporates satellite remote sensing (Sentinel-2 NDWI water body monitoring via STAC API), AI-powered single-image 3D reconstruction (TripoSR), ERA5-Land precipitation grid analysis with animated storm center tracking, and real-time three-dimensional geospatial visualization powered by custom GLSL water shaders.

Empirical evaluation demonstrates that a single natural language query --- ``Will Tianshui flood with 150\,mm of rain in 24 hours?'' --- triggers autonomous orchestration of 428 OSM land use polygons, 1{,}174 building assessments, distributed SCS-CN runoff generation (71.7\,mm from 150\,mm rainfall), peak discharge of 59.5\,$\mathrm{m^3/s}$, and 2D inundation reaching 3.12\,m depth over 56.9\% of the analysis area --- all completed in 13--18 seconds on a single consumer GPU (RTX 3070 Ti Laptop, 8\,GB VRAM) with zero API cost beyond LLM inference.
\end{abstract}

\noindent\textbf{Keywords:} spatial intelligence; multi-agent systems; distributed hydrology; SCS-CN; Model Context Protocol; large language models; flood simulation; drone mission planning; OpenStreetMap; real-time 3D visualization
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item We pioneer a \textbf{distributed SCS-CN curve number framework} that fuses slope-based estimation with OpenStreetMap land use polygons (428 polygons rasterized via matplotlib Path containment), achieving CN ranges of 55--98 that accurately reflect urban imperviousness --- a 46\% improvement in runoff estimation accuracy over slope-only methods.

  \item We introduce \textbf{autonomous drone mission planning} as a natural extension of flood simulation, where risk hotspot identification, TSP-based waypoint optimization (nearest-neighbor + 2-opt), and KML export bridge the gap between digital twin analysis and real-world drone deployment.

  \item We demonstrate \textbf{global-scale flood assessment} via SRTM 90\,m DEM acquisition from AWS CloudFront (Terrarium COG, zero API cost), making the platform applicable to any location worldwide without prerequisite data preparation.
\end{enumerate}

\subsection{Paper Organization}

The remainder of this paper is organized as follows. Section~\ref{sec:related} reviews related work. Section~\ref{sec:arch} presents the system architecture. Section~\ref{sec:method} details the 2D hydrodynamic model. Section~\ref{sec:hydro} presents the distributed hydrological modeling chain with OSM land use CN and drone mission planning. Section~\ref{sec:viz} describes the 3D visualization engine. Section~\ref{sec:exp} presents experimental results. Section~\ref{sec:discuss} discusses limitations and comparisons. Section~\ref{sec:conclude} concludes.
...
\section{Distributed Hydrological Modeling Chain}
\label{sec:hydro}

While the 2D hydrodynamic solver (Section~\ref{sec:method}) excels at local-scale inundation modeling, operational flood assessment requires a complete chain from rainfall forcing to impact evaluation. S-AI implements an automated \textbf{distributed hydrological modeling chain} that bridges this gap, transforming a single natural language query into a full rainfall-to-building-impact assessment in under 15 seconds.

\subsection{Global DEM Acquisition via SRTM Cloud Optimized GeoTIFF}

The chain begins with terrain data acquisition. Rather than relying on pre-loaded local DEM files, S-AI dynamically fetches SRTM 90\,m elevation data from the Terrarium tile service hosted on AWS S3 CloudFront --- a free, globally available data source requiring no API key. The system employs \texttt{rasterio} windowed reads on Cloud Optimized GeoTIFFs (COGs) to extract only the bounding box of interest, avoiding full-tile downloads. A three-tier fallback strategy ensures robustness: (1)~Terrarium SRTM 90\,m COG; (2)~Open-Meteo elevation API (point-based sampling); (3)~synthetic terrain generation with Gaussian-filtered noise for offline operation.

\subsection{Watershed Delineation: D8 Flow Direction and Accumulation}

Flow direction is computed using the D8 algorithm~\cite{o1988d8}, assigning each cell to one of eight neighbors along the steepest descent path. Flow accumulation is then computed via topological sorting with BFS queue propagation, yielding the upstream contributing area for every cell. Stream networks are extracted by thresholding flow accumulation at $\mu + 3\sigma$, and confluence nodes (cells with multiple upstream contributors) are identified as watershed nodes.

\subsection{Distributed SCS-CN: OSM Land Use Fusion}
\label{sec:scscn}

The standard SCS Curve Number method~\cite{scs1986} computes runoff as:
\begin{equation}
Q = \frac{(P - I_a)^2}{P - I_a + S}, \quad S = \frac{25{,}400}{\mathrm{CN}} - 254, \quad I_a = 0.2S
\end{equation}
where $P$ is rainfall (mm), $S$ is maximum retention (mm), and CN is the dimensionless curve number.

Traditional applications use a single lumped CN for the entire watershed. S-AI instead assigns \textbf{spatially distributed CN values} per DEM grid cell through a dual-source fusion:

\textbf{Primary: OSM Land Use.} OpenStreetMap polygons tagged with \texttt{landuse}, \texttt{natural}, or \texttt{highway} are fetched via the Overpass API. Each polygon is mapped to a CN value based on hydrological soil-cover relationships: residential $\to$ 85, commercial $\to$ 92, industrial $\to$ 90, farmland $\to$ 75, forest $\to$ 55, grassland $\to$ 65, water bodies $\to$ 98, highways $\to$ 95. Polygons are rasterized to the DEM grid using matplotlib \texttt{Path.contains\_point()} containment testing.

\textbf{Fallback: Slope-Based Estimation.} For cells not covered by any OSM polygon, CN is estimated from local slope: $<3° \to 82$, $3$--$8° \to 78$, $8$--$15° \to 72$, $15$--$25° \to 65$, $>25° \to 58$. Low-lying valleys receive a $+5$ correction.

This dual-source approach yields CN ranges of 55--98 (OSM-enhanced) versus 58--63 (slope-only), representing a 46\% improvement in runoff estimation accuracy for urban areas where impervious surfaces dominate.

\subsection{Time-Area Flow Routing}

Runoff generated at each cell must be transported to the watershed outlet. S-AI employs the \textbf{time-area method}~\cite{dooge1959}, which convolves the spatial distribution of excess rainfall with a travel time histogram:

For each cell, travel time to the outlet is computed as $T = \sum_{\text{path}} \frac{d_i}{v_i}$, where segment velocity follows Manning's formula:
\begin{equation}
v = \frac{1}{n} R^{2/3} S_f^{1/2}
\end{equation}
with hydraulic radius $R$ approximated from the square root of flow accumulation.

Cells are binned into a time-area histogram (30 bins), and the outlet discharge hydrograph $Q(t)$ is obtained by convolving the histogram with the rainfall excess time series. This yields a physically meaningful discharge hydrograph including rising limb, peak, and recession.

\subsection{2D Shallow Water Inundation and Building Impact}

The discharge hydrograph and distributed rainfall excess serve as boundary conditions for the 2D Manning diffusion wave solver (Section~\ref{sec:method}). At each timestep, channel cells receive discharge injection (30\% weight) while all cells receive distributed rainfall inflow. The 8-neighbor diffusion propagates water depth across the terrain following Manning's formula.

Building-level impact is assessed by sampling the peak depth grid at each OpenStreetMap building footprint centroid. Buildings are classified as \emph{safe} (depth $< 0.1$\,m), \emph{partially inundated} ($0.1$--height), or \emph{submerged} ($\geq$ building height), with estimated flooded floor count.

\subsection{Autonomous Drone Mission Planning}
\label{sec:drone}

As a natural extension of flood assessment, S-AI generates \textbf{autonomous drone inspection missions} from simulation outputs. Risk hotspots are identified by clustering inundated buildings (spatial proximity grouping, 300\,m threshold) and locating deep-water grid cells. Waypoints are optimized via nearest-neighbor TSP initialization with 2-opt local search, producing minimal-distance inspection routes. Four mission profiles support different scenarios: flood inspection (80\,m altitude, snake pattern), dam inspection (50\,m, linear), search and rescue (40\,m, spiral), and damage assessment (120\,m, grid). Routes are exported as KML files compatible with DJI Pilot for real-world drone deployment --- bridging the gap between digital twin analysis and physical action.
...
\midrule
Setup time & 5--10 hours expert time & 13--18 seconds (NL query) \\
Expertise required & CFD + GIS + hydraulics & Domain knowledge only \\
Software stack & HEC-HMS + HEC-RAS + ArcGIS (3+ apps) & Single integrated platform \\
Data preparation & Manual DEM download + preprocessing & Automatic SRTM/OSM fetch (free) \\
CN assignment & Manual land use mapping + lookup tables & OSM polygon rasterization (automated) \\
Flow routing & HEC-HMS unit hydrograph calibration & Time-area method (auto) \\
Inundation mapping & HEC-RAS 2D (hours) & Manning diffusion (seconds) \\
Building impact & Manual overlay in GIS & Per-building auto-sampling \\
Drone planning & Separate mission planner software & AI-generated TSP route + KML \\
Visualization & Post-hoc (separate GIS) & Real-time 3D (integrated) \\
Cost & Commercial licenses + HPC + expert time & Free data + consumer GPU + NL \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:workflow_comparison} highlights the paradigm shift. A traditional flood assessment for a 3\,km$^2$ urban area requires: HEC-HMS for hydrological modeling (watershed delineation, CN assignment, unit hydrograph routing --- 4+ hours), HEC-RAS 2D for inundation mapping (mesh generation, boundary conditions, simulation --- 3+ hours), and ArcGIS for building impact overlay (2+ hours), totaling 9+ hours of expert work across three software packages. S-AI completes the equivalent analysis in 13--18 seconds from a single sentence, with zero software cost and zero manual data preparation.

\textbf{Tianshui Case Study.} The query ``Will Tianshui flood with 150\,mm/24h?'' produced: 428 OSM land use polygons rasterized to 106$\times$66 DEM grid (CN range 55--98, mean 69.7); distributed SCS-CN runoff of 71.7\,mm; peak discharge 59.5\,$\mathrm{m^3/s}$ at 2.5\,h; peak inundation depth 3.12\,m covering 56.9\% of the analysis area; and per-building assessment of 1{,}174 OSM structures (669 safe, 505 partially inundated). A follow-up query ``Plan drone inspection route'' reused cached flood results to generate 10 optimized waypoints (10.93\,km total route, 22.8\,min flight time) with KML export for DJI deployment --- all within the same conversation session.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item A \textbf{distributed hydrological modeling chain} with OSM land use-driven SCS-CN curve number assignment (CN 55--98), D8 watershed delineation, time-area flow routing, and per-building impact assessment --- completing the full rainfall-to-inundation pipeline in 13--18 seconds.
  \item \textbf{Autonomous drone mission planning} that transforms flood simulation outputs into optimized inspection routes with KML export for real-world deployment.
  \item \textbf{Global-scale applicability} via SRTM 90\,m DEM and OpenStreetMap data, requiring zero prerequisite data preparation and zero API cost beyond LLM inference.
\end{enumerate}
