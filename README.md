...
"上传一段河流/洪水监控视频 → 📹 视频分析"
  → OpenCV抽帧 → YOLOv8人员/车辆检测 → GLM-4V逐帧水体识别 → 趋势分析 + 风险评估

"分析完成后 → 📋 生成报告"
  → GLM-4 AIR汇总所有分析数据 → 专业HTML报告(摘要/方法/发现/风险/建议) → 可打印PDF
```
...
### 12. 水务视频智能分析 (Video Analysis Engine)

上传无人机巡河/监控录像，系统自动**抽帧 + 逐帧AI分析**：

```
用户: 上传视频(mp4/avi/mov) → 点击📹按钮
      ↓
┌─ 三引擎混合分析 ───────────────────────────────────────────────┐
│                                                                │
│  ┌─ OpenCV 抽帧 ──────────────────────────────────────────┐    │
│  │  每3秒采样1帧，自动缩放至1280px以内                       │    │
│  └──────────────────────────────────────────────────────────┘   │
│                           ↓                                    │
│  ┌─ GLM-4V 逐帧语义分析 ──────────────────────────────────┐    │
│  │  每帧AI判断：                                             │    │
│  │  · 水体类型(河流/湖泊/水库/洪水/排水渠)                   │    │
│  │  · 水面状态(平静/流动/湍急/溢流)                         │    │
│  │  · 水面占比(0-100%)                                      │    │
│  │  · 安全隐患 + 风险等级(1-5)                               │    │
│  │  GLM-4V确认有水时才运行HSV标注水体位置                    │    │
│  └──────────────────────────────────────────────────────────┘   │
│                           ↓                                    │
│  ┌─ YOLOv8 目标检测 (GPU) ────────────────────────────────┐    │
│  │  RTX 3070 Ti 加速，每帧检测：                             │    │
...
- **多色域水体检测**：蓝(H:95-130)、绿(H:35-85藻类)、棕(H:10-30泥水)三段HSV色域，覆盖清水/浑水/洪水
- **GLM-4V门卫机制**：AI确认有水才画HSV标注，避免蓝建筑/阴影误判
- **中文渲染**：PIL ImageDraw + 微软雅黑字体，标注帧中文无乱码
- **GPU加速**：YOLOv8n在RTX 3070 Ti上推理，无需PyTorch CPU fallback

### 13. 智能报告生成 (Report Generation)

任意分析完成后，一键生成**咨询级专业报告**：
...
│                                                                │
│  输入数据源:                                                   │
│  · 工具调用结果 (allExportData)                                │
│  · 灾情评估 (disasterAssessment)                               │
│  · 视频分析 (videoAnalysis)                                    │
│  · 多情景对比 (comparisonResult)                               │
│  · 用户原始查询                                                │
│                                                                │
│  AI自主生成:                                                   │
│  ✅ 报告标题                                                   │
...
│  └──────────────────────────────────────────────┘             │
│                                                                │
│  Ctrl+P 打印为PDF | 自包含HTML，无外部依赖                    │
└────────────────────────────────────────────────────────────────┘
```

从"工具"升级为"顾问"——一份人工报告5-10万，S-AI 30秒生成。
...
| 📹 上传视频 → 视频分析 | YOLOv8+GLM-4V混合 | 抽帧→水体检测→人员/车辆YOLO检测→趋势分析 |
| 分析后 → 📋生成报告 | GLM-4 AIR报告引擎 | 汇总所有分析→HTML专业报告→可打印PDF |
...
│   │   ├── video_analysis.py        # 📹 视频分析引擎 (OpenCV+YOLOv8+GLM-4V 三引擎混合)
│   │   ├── report.py                # 📋 智能报告生成 (GLM-4 AIR → 专业HTML报告)
...
│   │   │   ├── files.py             # upload, image/video analysis, 报告生成
