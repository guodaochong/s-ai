...
"对比赤峰50mm 100mm 200mm暴雨洪水全过程"
  → 📊 多情景对比引擎 → 同一pipeline跑3套降雨参数 → 并排DAG + 指标对比表

"在地图上画一个框 → 🤖 全链路推演"
  → 🗺️ 地图交互 → AI识别bbox → 自主编排pipeline → 结果渲染到框选区域

"上传一张洪涝现场照片 → 🚨 灾情评估"
  → GLM-4V视觉推理 → 参照物估算水深 → 受损建筑/道路/隐患识别 → 建议措施 + 置信度
```
...
### 9. 多情景对比推演引擎 (Multi-Scenario Comparison)

同一区域，不同降雨参数，**一套pipeline跑N遍，结果并排对比**：

```
用户: "对比赤峰50mm 100mm 200mm暴雨洪水全过程"
      ↓
┌─ 情景解析 ─────────────────────────────────────────────────────┐
│  _extract_scenarios() 提取降雨参数:                              │
│  · 50mm (5年一遇) · 100mm (10年一遇) · 200mm (50年一遇)        │
│  · 自动推算重现期，支持"XX年一遇"和"XXmm"两种输入格式            │
└────────────────────────────────┬───────────────────────────────┘
                                 ↓
┌─ 并行执行 + 逐情景可视化 ──────────────────────────────────────┐
│                                                                │
...
│  └──────────┘  └───────────┘  └───────────┘                   │
│                                                                │
│  每个情景独立mini-DAG卡片，活跃卡片高亮，完成后变绿               │
└────────────────────────────────┬───────────────────────────────┘
                                 ↓
┌─ 对比结果表 ───────────────────────────────────────────────────┐
│  指标           50mm      100mm      200mm                     │
│  最大水深       0.8m      1.5m       2.5m  ← 最大值金色高亮    │
│  淹没面积       5.2km²    12.8km²    25.3km²                   │
│  受灾人口       1,200     4,500      12,000                    │
│  经济损失       800万     3,200万    8,500万                    │
└────────────────────────────────────────────────────────────────┘
```

`_extract_metrics()` 从 flood_sim_3d、flood_assessment、runoff_compute、flood_risk_zones 等工具结果中提取可比指标，`_build_comparison()` 汇总成对比表并计算差异百分比。

### 10. 地图交互推演 (Map Draw → AI Deduction)

用户不再需要打字描述区域——**直接在地图上画框，AI自主分析**：

```
用户操作: ✏️ 画框分析 → 框选区域 → 点击按钮
          ↓
  ┌──────────────────────────────────────────────────────┐
  │  🤖 全链路推演  →  AI编排pipeline，bbox传入每步工具    │
  │  📊 多情景对比  →  对框选区域跑多套降雨参数            │
  │  🌊 会淹吗？    →  单次洪水淹没分析                   │
  │  🏙️ 识别建筑   →  OSM建筑提取                        │
  │  🌧️ 降雨过程   →  面雨量网格分析                     │
...
`_extract_bbox()` 从query中解析 `[west,south,east,north]` 坐标，`_build_args()` 将bbox传递给 precipitation_grid、flood_sim_3d、flood_inundation_map、building_extract 等空间工具。

### 11. GLM-4V 灾情照片智能评估 (Disaster Photo Assessment)

上传一张洪涝/灾害现场照片，GLM-4V进行**结构化灾情评估**：
...
│                                                                │
│  🔍 参照物估算水深:                                             │
│     · 车辆轮胎 ~0.4m → 车身 ~1.5m → 门框 ~2m → 一层楼 ~3m     │
│                                                                │
│  📊 结构化输出:                                                 │
│     · 灾害类型: 内涝/洪水/管涌/溃坝/山洪                        │
│     · 严重程度: 1-5级 (1=轻微, 5=特大)                         │
...
│  │  [原图缩略图 - 点击放大]               │                     │
│  └──────────────────────────────────────┘                     │
│  灾害类型: 洪水    估算水深: 1.5m    受影响人口: 数十人        │
│  📏 水深依据: 车辆轮胎、车身、门框高度                          │
│  🏚️ 受损建筑: 商业建筑×3 部分进水                              │
│  ⚠️ 安全隐患: [车辆移动风险] [结构受损] [电气损坏]              │
│  📋 建议措施:                                                  │
│     ① 立即疏散居民和企业员工                                    │
│     ② 封闭主要道路                                              │
│     ③ 切断电源防止触电                                          │
...
- **关键词触发**：30+灾情关键词（灾情/洪水/淹没/评估/损失...）自动切换到专业评估模式
- **图片快捷按钮**：上传图片后显示 🚨灾情评估 / 🏗️3D重建 / 🔍通用分析 三个按钮，用户意图明确，避免LLM误判
- **图片自适应**：`_resize_image_b64()` 通过 PIL 自动缩放到1024px内，适配GLM-4V的 payload 限制
- **类型自适应**：GLM-4V返回的字段类型可能不固定（水深可能是数字或对象），前端格式化器自适应处理
...
| `对比赤峰50mm 100mm 200mm暴雨洪水全过程` | 📊 多情景对比 | 同一pipeline跑3套参数，并排DAG + 指标对比表 |
| `模拟赤峰暴雨洪水全过程` | 🌊 空间推演引擎 | AI编排: DEM→设计暴雨→径流→洪水模拟→损失评估 |
| 🗺️ 画框 → 🤖全链路推演 | 地图交互推演 | 框选区域→AI编排pipeline→bbox传工具→结果渲染到框 |
| 📷 上传照片 → 🚨灾情评估 | GLM-4V视觉评估 | 参照物估水深→建筑/道路受损→安全隐患→建议措施 |
...
│   │   ├── streaming.py             # SSE chat endpoint + ReAct推理循环 + pipeline + 多情景 + 灾情评估
│   │   ├── pipeline.py              # 🌊 空间推演引擎 (AI自主编排DAG + 多情景对比 + bbox提取)
...
│   │   ├── multimodal.py            # GLM-4V 图像分析 + 🚨 灾情照片智能评估 (结构化JSON)
