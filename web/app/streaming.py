...
async def chat_stream(q: str, history: str = "", workflows: str = ""):
    ...
    async def _generate():
        ...
        # ── 4a. Multi-scenario comparison — N pipelines with different rainfall ──
        scenarios = _extract_scenarios(message)
        if scenarios and len(scenarios) >= 2:
            yield sse({"type": "thinking_start", "agent": "planner", "label": "📊 多情景对比分析"})
            pipeline_tpl = await detect_pipeline(message)
            yield sse({"type": "thinking_end", "agent": "planner"})
            if pipeline_tpl:
                loc = _extract_location(message)

                async def _ms_exec(tool_name: str, args: dict, user_msg: str):
                    server = TOOL_TO_SERVER.get(tool_name, "")
                    return await _execute_single_tool("multi_scenario", tool_name, server, args, user_msg, trace)

                tools_used: list[str] = []
                async for event in execute_multi_scenario(pipeline_tpl, scenarios, message, loc, trace, _ms_exec):
                    yield event
                    if isinstance(event, str) and event.startswith("data:"):
                        try:
                            evt = json.loads(event[5:].strip())
                            if evt.get("type") == "tool_result" and evt.get("tool"):
                                tn = evt["tool"]
